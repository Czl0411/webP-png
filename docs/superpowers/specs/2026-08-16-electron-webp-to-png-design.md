...
- 递归转换源文件夹及全部子文件夹内的 `.webp` 或 `.WEBP` 文件。
- 用户选择源文件夹和输出文件夹；同名 PNG 直接覆盖。
- 输出目录保留源文件夹的子目录层级，避免不同子目录的同名文件互相覆盖。
...
2. 用户选定两个目录后，点击转换。
3. 主进程验证两个目录、递归筛选 WebP 文件，依次用 sharp 写出同名 PNG，并在输出目录重建相对路径。
...
- 单元测试覆盖递归扫描、输出目录层级保留、成功转换、错误不中断、目录验证。
